...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def __init__(self, engine: Engine):
        ...
        player = self.engine.player
        engine.mouse_location = player.x, player.y #May need to switch to int(player.x), int(player.y) based on int(event.tile.x), int(event.tile.y)

    def on_render(self, console: tcod.Console) -> None:
        super().on_render(console)
        x, y = self.engine.mouse_location #Here as well
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action]:
        ...
        if key in MOVE_KEYS:
            ...
            self.engine.mouse_location = x, y # Possible bug here as well
            return None
